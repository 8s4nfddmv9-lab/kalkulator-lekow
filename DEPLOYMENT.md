...
Przed buildem `tool/prepare_web_fallback_fonts.py` pobiera przypięte pliki Roboto Regular oraz Noto Sans Symbols WOFF2 i odrzuca każdy plik o innym rozmiarze, sygnaturze lub SHA-256. Bootstrap ustawia `fontFallbackBaseUrl: 'fallback-fonts/'`, dlatego CanvasKit nie potrzebuje `fonts.gstatic.com`. Odpowiednie licencje OFL są publikowane obok fontów, a pełne informacje znajdują się w `THIRD_PARTY_NOTICES.md`.

Przed publikacją `tool/finalize_web_pwa.py` wymaga kompletnego lokalnego renderera CanvasKit oraz obu zweryfikowanych fallbacków fontów i ich licencji. `tool/smoke_test_offline_pwa.py` czyści zwykły HTTP cache, odcina serwer i sieć, odrzuca rzeczywiście żądane zewnętrzne zasoby startowe oraz potwierdza uruchomienie wyłącznie z CacheStorage service workera. GitHub Pages nie zostanie wdrożony, jeżeli którakolwiek z tych bramek zawiedzie.
